...
class Method:
    ...
    def processreply(self, reply, **kwargs):
        ...
        if not self.faults():
            try:
                return client.processreply(reply)
            except WebFault, e:
                return (500, e)
        else:
            return client.processreply(reply)

    def processtransporterror(self, e, **kwargs):
        """
        Process a reply asynchrounously
        @param reply: A reply object
        @type: suds.transport.Reply
        ...
        if not self.faults():
            try:
                return client.processtransporterror(e)
            except WebFault, e:
                return (500, e)
        else:
            return client.processtransporterror(e)
...
class SoapClient:
    ...
    def send(self, soapenv):
        ...
        try:
            ...
            result = self.processreply(reply)
    ...
    # DRo
    # Process errors (exceptions) during transport
    ...
    def processreply(self, reply):
        ...
        log.debug('Processing asynchronous reply:\n%s', reply)
    ...
    def last_sent(self, d=None):
        ...
        if d is None:
            ...
            # Storing the last_sent has no meaning in asynchronous mode
    ...
    def last_received(self, d=None):
        ...
        if d is None:
            ...
            # Storing the last_received has no meaning in asynchronous mode
            if self.options.async == False:
                messages[key] = d
